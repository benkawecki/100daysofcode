...
def get_state_abbrev(state_name, us_state_abbrev=us_state_abbrev):
    ...
    if state_name in us_state_abbrev:
        abr = us_state_abbrev[state_name]
    else:
        abr = NOT_FOUND
    return abr
...
def combine_state_names_and_abbreviations(us_state_abbrev=us_state_abbrev,
    ...
    """Get the first 10 state abbreviations ('AL', 'AK', 'AZ', ...) from
       ...
       alphabetical order"""
    first_ten = us_state_abbrev.values()
    first_ten = [s for s in us_state_abbrev.values()][0:9]
    states.sort()
    last_ten = states[-9:]
...
if __name__ =="__main__":
    ...
    print(get_state_abbrev('Maryland'))
    print(get_longest_state(us_state_abbrev), '+ ', get_longest_state(states))
